...
const EDITORIAL_BY_ID: Record<string, StatusEditorial> = {
  ...
  twitter: {
    humanSummary:
      "Xは「閲覧」はできるのに「投稿／通知／DM」だけ失敗するなど、部分障害の形で出やすいサービスです。",

    whatItMeansIfDown:
      "このページが「オフライン」でも、X側の全体停止とは限りません。回線・DNS・一時的なアクセス制限で失敗することがあります。",

    whatToCheckNext: [
      "タイムラインは表示されるが、投稿・リプライ・いいねの反映だけ遅れていないか確認します。",
      "通知だけ遅延、またはDMだけ不安定という形で分かれていないか確認します。",
      "画像読み込み（メディア）だけ遅い／欠けるなど、機能別の影響がないか確認します。",
    ],
    ...
    usefulWhen: [
      "外から見てXに到達できるか（HTTP到達性）を確認したいとき",
      "“投稿できない”などの前に、まず到達性を見たいとき",
    ],
